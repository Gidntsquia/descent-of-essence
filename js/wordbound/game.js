...
(function () {
  ...
  // AUDIO ticket (GOALS.md, "NO SOUND AT ALL", 2026-08-21, part 2 -- iOS
  // hardware ringer/silent switch): iOS Safari silences WebAudio-only pages
  // when the ring/silent switch is set to silent, with no JS-visible signal
  // for that switch's position at all -- .state stays 'running', gain
  // values are correct, nodes really do get scheduled, and it's STILL
  // silent on the device. The documented workaround is getting the page's
  // audio session into the 'playback' category instead of the default
  // 'ambient' category (which iOS deliberately routes through the ringer
  // switch). A WebAudio-only page never triggers that category change on
  // its own. Two mitigations, applied together since browser support for
  // the first is still new:
  //   1. navigator.audioSession (Safari 17+): sets the category directly
  //      where it exists.
  //   2. The long-standing "silent looping <audio> element" trick, which
  //      works everywhere: giving the page a genuine HTMLMediaElement that
  //      is actually, continuously playing (not muted, not volume-0 --
  //      either of those would defeat the category trick, since iOS keys
  //      off real playback activity, not element presence) gets the same
  //      'playback' category from a real <audio> element instead of an
  //      API most browsers don't have yet. The element's *content* is
  //      silence (every PCM sample is the zero-amplitude midpoint), so
  //      nothing is audibly played -- only its playback state matters.
  //   Neither mitigation is verifiable from this sandbox (no real iOS
  //   hardware, and the ringer switch has no JS-visible signal even on a
  //   real device) -- see PROGRESS.md for exactly what was and wasn't
  //   confirmed.
  ...
  function primeIosAudioPlaybackCategory() {
    try {
      if (navigator.audioSession) navigator.audioSession.type = 'playback';
    } catch (e) { /* not supported on this browser -- nothing to do */ }
    try {
      if (!silentAudioEl) {
        silentAudioEl = new Audio(SILENT_AUDIO_DATA_URI);
        silentAudioEl.loop = true;
        silentAudioEl.volume = 1; // silent by content, not by volume/mute -- see comment above
      }
      if (silentAudioEl.paused) silentAudioEl.play().catch(function () {}); // best-effort, re-tried on every gesture
  ...
  Game.init = function () {
    ...
    // from the gesture, e.g. after other synchronous work).
    //
    // AUDIO follow-up (Jaxon iPhone report, 2026-08-21, "still not hearing
    // any sound" after the fix above shipped): this used to remove its own
    // listeners after the first gesture. If that first resume() got
    // refused, or iOS later suspends/interrupts the context mid-session
    // (backgrounding the app, a phone call, Safari tab restore -- all
    // things that happen well after page load, not just on it), there was
    // no listener left to retry. resume() on an already-'running' context
    // is a harmless no-op (see initAudioContext's own comment), so it's
    // cheap to keep re-attempting on every real gesture for the life of
    // the page instead of firing once and going away.
    var primeAudioOnce = function () {
      try { initAudioContext(); } catch (e) { /* AudioContext unsupported -- nothing to do */ }
      primeIosAudioPlaybackCategory();
    };
